...
Protoplus.ui = {
	isVisible: function (element)
	{
		...
		if (element.getCurrentStyle("display") == "none" || element.getCurrentStyle("visibility") == "hidden")
		{
			return false;
		}
	...
	setContextMenu: function (element, options)
	{
		...
		var createInnerList = function (cont, itemConf)
		{
			...
			cont.mouseEnter(function ()
			{
				...
				container.style.left[pos] = '-' + (container.getWidth() - 5) + 'px';
			}, function ()
			{
				clearTimeout(t);
			...
			if (local || (Prototype.Browser.Opera && e.ctrlKey) || Event.isRightClick(e) || Prototype.Browser.IE)
			{
				...
				if (element !== false)
				{
					...
					container.setStyle(
					{
						...
						left: '3px',//(opt.left ? opt.left : left) + 'px'
					});
	...
	slider: function (element, options)
	{
		...
		{});
		if ("Userform" in window && "url" in Userform)
		{
			options.buttonBack = 'url("' + Userform.url + 'images/ball.png") no-repeat scroll 0px 0px transparent';
		}
	...
	spinner: function (element, options)
	{
		...
		//Checks to see if the value ATTRIBUTE has been set in the HTML (not dynamically via javascript). This indicates whether the value has been set via XML.
		...
		element.writeAttribute('autocomplete', 'nope');
		var buttonStyles = {
			height: '10px',
			cursor: 'default',
			textAlign: 'center',
...
{
	return Co.Translator.Translate(this);
};
// SITES-1560 - helper object to hold processing offset gift donation
var totalAmountHelper = {
	init: function () {
		for (var i in formJson) {
			if (i.split('_')[1] === 'type' && formJson[i] === 'control_totalamount') {
				this._exists = true;
				this._id = i.split('_')[0];
			}
		}
		...
		if (typeof this._id === 'undefined')
			this._id = 0;
		if (this._exists === true && this._id !== 0) {
			this._offsetGiftEnabled = formJson[this._id + '_offsetGiftEnabled'] === "Yes";
			this._offsetGiftPercent = 0;
			if (this._offsetGiftEnabled && !isNaN(formJson[this._id + '_offsetGift'])) {
				this._offsetGiftPercent = new Number(formJson[this._id + '_offsetGift']).valueOf() / 100;
				// Not the best way to deal with this, but dealing with observers and where to set them up is well beyond the SP
				if (typeof $j !== "undefined") {
					$j("#input_" + this.id()).off("click");
					$j("#input_" + this.id()).on("click", function (ev) {
						if (typeof updateTotalSchedular !== 'undefined') {
							updateTotalSchedular.runTask();
						}
					});
				}
			}
	...
	offsetGiftEnabled: function () {
		if (typeof this._offsetGiftEnabled === 'undefined') {
			this.init();
		}
		return this._offsetGiftEnabled;
	},
	offsetGiftPercent: function () {
		if (typeof this._offsetGiftPercent === 'undefined') {
			this.init();
		}
		return this._offsetGiftPercent;
	},
	offsetGiftChecked: function () {
		var checkbox, result = false;
		if (this.offsetGiftEnabled()) {
			checkbox = document.getElementById("input_" + this.id());
			if (checkbox !== null) {
	...
	offsetGiftToggleVisibility: function (visible) {
		if (totalAmountHelper.exists() && this.offsetGiftEnabled()) {
			var element = document.getElementById("div_offset_gift_" + this.id());
			var checkbox = document.getElementById("input_" + this.id());
			...
			if (!visible) this.offsetGiftSetValue(0);
			if (typeof updateTotalSchedular !== 'undefined') {
				updateTotalSchedular.runTask();
			}
		}
	},
	offsetGiftSetValue: function (amount) {
		if (totalAmountHelper.exists() &&
			totalAmountHelper.offsetGiftEnabled() &&
			typeof $j !== "undefined") {
			$j("#hidden_" + this.id()).val(Number(amount).toFixed(2).toString());
		}
	}
};
//---
var payformSettings = {
	init: function () {
		for (var i in formJson) {
			if (i.split('_')[1] === 'type' && formJson[i] === 'control_payform') {
				this._exists = true;
				this._id = i.split('_')[0];
			}
		}
		...
		if (typeof this._id === 'undefined')
			this._id = 0;
		if (this._exists === true && this._id != 0)
			this._options = formJson[this._id + '_options'];
		else
			this._options = null;
	},
	exists: function () {
	...
	/**
	 * User is required to enter payment information even when he is not making a donation right now.
	 * In general, we only require payment information if user is making a payment right now, but the Shliach can choose to always require
	 * a payment method, for later offline billing as needed.
	 */
	isAlwaysRequired: function () {
		return formJson[this._id + "_required"] == "Yes";
	}
};
var formCurrency = {
		init: function () {
			for (var i in formJson) {
				if (i.split('_')[1] === 'type' && formJson[i] === 'control_payform')
				{
					this._hasPayform = true;
					var currencyData = formJson[i.split('_')[0]+'_options'].currency.split('|');
					if (currencyData[0] === 'default') 
					{
						break;
					}
					this._isDefault = false;
					this._symbol = currencyData[1];
					this._value = currencyData[2];					
					break;
				}
			}
			...
			{				
				...
				this._symbol = defaultCurrency.symbol;
				this._value = defaultCurrency.value;
			}
		},
		isDefault: function () {if (typeof this._isDefault === 'undefined') {this.init();} return this._isDefault;},
		symbol: function () {if (typeof this._symbol === 'undefined') {this.init();} return this._symbol;},
		value: function () {if (typeof this._value === 'undefined') {this.init();} return this._value;},
	};


var Userform = {
	url: "//www.chabad.org/",
	server: "//www.chabad.org/templates/article.asp?aid=" + Co.ArticleId,
	conditions: {},
	calculations: {},
	condValues: {},
	progressBar: false,
	forms: [],
	saveForm: false,
	uploadAllowedExtensions: ["jpeg", "jpg", "gif", "png", "pdf"],
	...
	texts:
	{
		confirmEmail: 'E-mail does not match'.locale(),
		pleaseWait: 'Please wait...'.locale(),
		confirmClearForm: 'Are you sure you want to clear the form'.locale(),
		lessThan: 'Your score should be less than or equal to'.locale(),
		incompleteFields: 'There are incomplete required fields. Please complete them.'.locale(),
		required: 'This field is required.'.locale(),
		requireOne: 'At least one field required.'.locale(),
		requireEveryRow: 'Every row is required.'.locale(),
		requiredGroupInput: 'Please fill out all required fields in this section'.locale(),
		email: 'Enter a valid e-mail address'.locale(),
		alphabetic: 'This field can only contain letters'.locale(),
		numeric: 'This field can only contain whole numbers, no decimals.'.locale(),
		alphanumeric: 'This field can only contain letters and numbers.'.locale(),
		url: 'This field can only contain a valid URL'.locale(),
		html: 'This field can not contain any HTML'.locale(),
		uploadExtensions: 'You can only upload following files:'.locale(),
		uploadFilesize: 'File size cannot be larger than'.locale(),
		gradingScoreError: 'Score total should only be less than or equal to'.locale(),
		inputCarretErrorA: 'Input should not less than the minimum value:'.locale(),
		inputCarretErrorB: 'Input should not greater than the maximum value:'.locale(),
		maxDigitsError: 'The maximum digits allowed is'.locale(),
		freeEmailError: 'Free email accounts are not allowed'.locale(),
		minSelectionsError: 'The minimum required number of selections is '.locale(),
		maxSelectionsError: 'The maximum number of selections allowed is '.locale(),
		pastDatesDisallowed: 'Date must not be in the past.'.locale(),
		multipleFileUploads_typeError: '{file} has invalid extension. Only {extensions} are allowed.'.locale(),
		multipleFileUploads_sizeError: '{file} is too large, maximum file size is {sizeLimit}.'.locale(),
		multipleFileUploads_minSizeError: '{file} is too small, minimum file size is {minSizeLimit}.'.locale(),
		multipleFileUploads_emptyError: '{file} is empty, please select files again without it.',
		multipleFileUploads_onLeave: 'The files are being uploaded, if you leave now the upload will be cancelled.'.locale(),
		generalError: 'There are errors on the form. Please fix them before continuing.'.locale(),
		generalPageError: 'There are errors on this page. Please fix them before continuing.'.locale(),
		paymentMethod: 'Select a payment method.'.locale(),
		paymentMinimum: '{amount} is less than the minimum of {minimum}.'.locale(),
		creditCardError: 'Please fill out all required fields.'.locale(),
		invalidCardTypeError: 'We\'re sorry, the card type you are entering is not allowed.'.locale(),
	},
	paymentTexts:
	{
		couponApply: 'Apply'.locale(),
		couponChange: 'Change'.locale(),
		couponEnter: 'Enter Coupon'.locale(),
		couponExpired: 'Coupon is expired. Please try another one.'.locale(),
		couponInvalid: 'Coupon is invalid. Please try another one.'.locale(),
		couponValid: 'Coupon is valid.'.locale(),
		couponBlank: 'Please enter a coupon.'.locale(),
	...
	{ return; //CCOEVENTS-179. deprecated
		var form = $$('.userform-form')[0];
		var action;
		if (form)
		{
	...
	createConsole: function ()
	{
		...
		if (Userform.debug)
		{
			Userform.debugOptions = document.readJsonCookie('debug_options');
		}
	},
	init: function (callback)
	{
		var ready = function ()
		{
			BuildSource.init();//Handles visibility of certain form elements.

			var expYear = $$('[name$="[cc_exp_year]"]')[0]; // dynamically update years for exp date -ML
			if (expYear)
			{
				var value = expYear.value;
				expYear.options.length = 0;
				var today = new Date().getFullYear(), endYear = today+10;
				expYear.options.add(new Option(''));
				for (var i=today; i < endYear; i++)
					expYear.options.add(new Option(i, i));
			...
			try
			{
				...
				this.createConsole();
				//this.getServerURL(); //CCOEVENTS-179.
				if (callback)
				{
					callback();
				}
				...
				{
					if (form.name == "form_" + form.id || form.name == "q_form_" + form.id || form.getAttribute('data-type') == 'new_form' )
					{
						if( !form.id )
						{
							/*
								CWNP-557 - Embedded forms may not have an id, but a unique id is required for form validation (and data re-population).
								This unique number may not be *absolutely* necesary...
							*/
							var index = 1;
							while( document.getElementById( 'new_form_' + index ) )
								index += 1;
							form.setAttribute( 'id', 'new_form_' + index );
						}
						this.forms.push(form);
					}
				}.bind( this ) );
				updateTotals = initTotal();
				...
				// Delay checking all conditions in case fields will be filled by the browser
				setTimeout(function () {
					Userform.checkAllConditions();
				}, 250);
			}
			catch (err)
			{
			...
			//if (typeof formPostDataObject !== 'undefined') PopulateFormData(formPostDataObject); //Not defined
	...
	handleIFrameHeight: function ()
	{
		...
		else if ($$('ul > .form-section').length > 1)
		{
			var maxHeight = 0;
			var body = $$('body')[0];
		...
		try
		{
			if (this.ie() <= 8 && navigator.appVersion.indexOf('NT 5.'))
			{
				$A(this.forms).each(function (form)
				{
					...
					{
						form.action = form.action.replace(/\/\/submit\..*?\//, "//secure.chabad.org/");
					}
				});
			}
		}
	...
	initMultipleUploads: function ()
	{
		...
		$$('.form-upload-multiple').each(function (file)
		{
			...
			var f = Userform.getForm(file);
			var formID = f.formID.value;
			var uniq = formID + "_" + Userform.uniqueID;
			...
			if (className.include("validate[required]"))
			{
				...
				{
					if (!Userform.isVisible(parent))
					{
						Userform.corrected(parent);
						return true;
					}
					if (parent.select('.qq-upload-list li').length < 1)
					{
						Userform.errored(parent, Userform.texts.required);
						return false;
					}
					else
					{
			...
			{
				debug: Userform.debug,
				element: parent,
				action: Userform.server,
				subLabel: subLabel,
				buttonText: buttonText,
				sizeLimit: Userform.uploadMaxSize,
				allowedExtensions: Userform.uploadAllowedExtensions,
				messages:
				{
				...
				{
					Userform.errored(parent, message);
	...
	{
		if (Userform.currentSection)
		{
			Userform.currentSection.select('.form-pagebreak')[0].insert(new Element('div',
			{
				className: 'form-saving-indicator'
			}).setStyle('float:right;padding:21px 12px 10px').update('<img src="' + Userform.url + 'images/ajax-loader.gif" align="absmiddle" /> Saving...'));
		}
		setTimeout(function ()
		{
		...
		if (!$('hidden_submit_form'))
		{
			...
			{
				Userform.makeUploadChecks();
				$$('.form-saving-indicator').invoke('remove');
				Userform.saving = false;
			});
			$(document.body).insert(iframe);
		}
		$('current_page').value = Userform.currentSection.pagesIndex;
	...
	makeUploadChecks: function ()
	{
		...
		var a = new Ajax.Jsonp(Userform.url + 'server.php',
		{
			parameters:
			{
			...
			onComplete: function (t)
			{
				...
				if (res.success)
				{
					if (res.submissionID)
					{
						...
						try
						{
							Userform.editMode(res);
						}
						catch (e)
						{
							console.error(e);
						}
						Userform.openInitially = res.currentPage - 1;
					}
				}
			}
		});
	...
	setFocusEvents: function ()
	{
		$$('.form-radio, .form-checkbox').each(function (input) {
			// SITES-1619: update focus based on change, because in campus there's no mouse down on radio buttons;
			// they are hidden, and a custom one is used.
			$j(input).on('mousedown change', function () {
				Userform.lastFocus = input;
			});
		});
	...
	disableAcceptonChrome: function ()
	{
		...
		$$('input.form-upload').each(function (input)
		{
	...
	setCalendar: function (id, startOnMonday)
	{
		try
		{
			var calendar = Calendar.setup(
			{
				...
				dateField: "year_" + id,
				closeHandler: Userform.calendarClose,
				selectHandler: Userform.formatDate,
			...
			{
				Userform.calendarClose(calendar);
			});
	...
	getDefaults: function ()
	{
		$$('.form-textbox, .form-dropdown, .form-textarea').each(function (input)
		{
			...
			Userform.defaultValues[input.id] = input.value;
		});
		$$('.form-radio, .form-checkbox').each(function (input)
		{
			if (!input.checked)
	...
	handleOtherOptions: function ()
	{
		$$('.form-radio-other-input, .form-checkbox-other-input').each(function (inp)
		{
			...
			// SITES-1619: Also validate other fields when the other input is blurred.
			// Usually, this works because the other radio loses focus.
			// On campus, though, the other radio never has focus; it's hidden, and we use a custom one.
			$j(inp).on('blur', function() {
				inp.validateInput && inp.validateInput();
			});

			// Whenever one of the radio/check options changes, check weather an empty other field was disabled and is no
			// longer an error.
			var inputParent = $j(inp).closest('.form-radio-item, .form-checkbox-item');
			inputParent.on('change', function (e) {
				var other = inputParent.find('.form-radio-other, .form-checkbox-other')[0];

				if (!other.checked) {
					Userform.corrected(other);
				}
			});
		});
		$$('.form-radio, .form-checkbox').each(function (input)
		{
			var id = input.id.replace(/input_(\w+)_\d+/gim, '$1');
			if (id.match('other_'))
			{
				id = input.id.replace(/other_(\w+)/, '$1');
			}
			if ($('other_' + id))
			{
				var other = $('other_' + id);
				...
				{
					//Sets other to the placeholder value
					//other_input.value = other_input.value || other_input.getAttribute('data-otherHint');
				});
				other_input.observe('click', function ()
				{
					other_input.value = other_input.value === other_input.getAttribute('data-otherHint') ? 'bla' : other_input.value;
				});
				...
				{
					if (other.checked)
						otherOption.triggerEvent("click");
				}
			}
		});
	},
			...
			var dateTimeCheck = function ()
			{
				...
				if (erroredElement)
				{
					if (erroredElement === hourElement || erroredElement === minElement)
					{
						...
						Userform.errored(erroredElement, 'Enter a valid time');
					}
					else
					{
						erroredElement.errored = false;
						Userform.errored(erroredElement, 'Enter a valid date');
					}
				...
				{
					Userform.corrected(monthElement);
					Userform.corrected(dayElement);
					Userform.corrected(yearElement);
					if (hourElement && minElement)
					{
						Userform.corrected(hourElement);
						Userform.corrected(minElement);
					}
	...
	handleTextareaLimits: function ()
	{
		$$('.form-textarea-limit-indicator span').each(function (el)
		{
			...
			var countText = function ()
			{
				...
				if (count > limit)
				{
					// Highlight the character limit for a second.
					...
					// The new size of the input is the limit.
					var newSize = limit;

					// But if the limit type is words, calculate new size based on words.
					if (limitType == 'Words') {
						var wordString = contents.replace(/\s(\s+)?/g, '\u2622$1');
						var indexOfLastWord = 0;
						for (var numWords = 0; numWords < limit; numWords++) {
							indexOfLastWord = wordString.indexOf('\u2622', indexOfLastWord + 1);
						}

						newSize = indexOfLastWord;
					}

					input.value = contents.substring(0, newSize);
	...
	{
		$H(Userform.autoCompletes).each(function (pair)
		{
			var el = $(pair.key);
	...
	editMode: function (data, noreset, skipFields)
	{
		...
		var populateData = function (res)
		{
			if (!noreset)
			{
				$A(Userform.forms).each(function (frm)
				{
					Userform.resetForm(frm);
				});
			}
			$H(res.result).each(function (pair)
			{
				...
				try
				{
					...
					case "control_fileupload":
						...
						setTimeout(function ()
						{
							...
							else
							{
								...
								var ext = Userform.getFileExtension(filename);
								if (ext !== undefined)
								{
									if (Userform.imageFiles.include(ext.toLowerCase()))
									{
					...
					case "control_hidden":
						if ($('input_' + qid))
						{
							if (Userform.saveForm || document.get.mode == 'edit')
							{
					...
					case 'control_authnet':
						...
						Userform.countTotal(Userform.prices);
						break;
					case 'control_email':
			...
			$H(Userform.fieldConditions).each(function (pair)
			{
				var field = pair.key;
			...
			var a = new Ajax.Request('server.php',
			{
				...
				onComplete: function (t)
				{
					...
					if (res.success)
					{
						...
						Userform.getContainer($$('.form-captcha')[0]).hide();
						if (document.get.mode == 'submissionToPDF')
						{
							...
							var a = new Ajax.Request('server.php',
							{
								...
								onComplete: function (t)
								{
									...
									var formElement = $$('.userform-form')[0];
	...
	{
		conditions.reverse();
		Userform.conditions = conditions;
		conditions.each(function (condition)
		{
		
			// SITES-2384: We used to support only a single action, called "action".
			// Continue to use that if present.
			// Note that in the form settings, the property is called actions, but userform has always copied the settings
			// to an array called action.
			// We continue to use the name action in this file to minimize the possibility of breaking things.
			condition.action = condition.action ? [ condition.action ] : condition.actions || [];
		});
	},
	setCalculations: function (calculations)
	{
		Userform.calculations = calculations;
	},
	...
	{
		if (Userform.otherConditionTrue(field, 'hide')) return;
		var element = null;
		...
		if (Userform.getInputType(field) === 'widget')
		{
			Userform.showWidget(field);
		}
	...
	storeValue: function (field)
	{
		...
		// SITES-2749: Prevent recursive checkAllConditions below when 2 file lists are not the same instance.
		if ($j(id).val() instanceof FileList)
		{
			return;
		}
		...
		if ($j(id).val()) {
			...
			$j(id).data("value", $j(id).val());
			$j(id).val("");
			if ($j(id).val() != $j(id).data("value")) {
				Userform.checkAllConditions();
				updateValue(field);
				if (typeof updateTotalSchedular !== 'undefined') {
					updateTotalSchedular.runTask();
	...
	checkValueByOperator: function (operator, condValueOrg, fieldValueOrg)
	{
		...
		case "equalDay":
			return Userform.getDayOfWeek(fieldValue) == condValue;
		case "notEquals":
		case "notEqualDate":
		...
		case "after":
			return fieldValueOrg >= condValueOrg;
		default:
			Userform.error("Could not find this operator", operator);
		}
		return false;
	},
	...
	{
		if (Userform.typeCache[id])
		{
			return Userform.typeCache[id];
		}
		var type = false;
		if ($('input_' + id))
		{
	...
	strToDate: function (str)
	{
		...
		var d = new Date();
		// We don't want the date to be set to a particular hour etc unless included in the string.
		d.setHours(0);
		d.setMinutes(0);
		d.setSeconds(0);
	...
	checkCondition: function (condition)
	{
		...
		$A(condition.terms).each(function (term)
		{
			...
			try
			{
				var fieldType = Userform.getInputType(term.field);
				switch (fieldType)
				{
				case "combined":
					if (['isEmpty', 'isFilled'].include(term.operator))
					{
						...
						if (Userform.checkValueByOperator(term.operator, term.value, filled))
						{
							any = true;
						}
						else
				...
				case "birthdate":
				case "datetime":
					value = (fieldType == "datetime") ? Userform.getDateValue(term.field) : Userform.getBirthDate(term.field);
					if (value === undefined)
					{
						return;
					}
					if (['isEmpty', 'isFilled'].include(term.operator))
					{
						if (Userform.checkValueByOperator(term.operator, term.value, value))
						{
							any = true;
						}
						else
					...
					else
					{
						...
						{
							if (Userform.checkValueByOperator(term.operator, Userform.strToDate(termValue), Userform.strToDate(value.split('T')[0])))
							{
								any = true;
							}
							else
						...
						{
							if (Userform.checkValueByOperator(term.operator, termValue, Userform.strToDate(value)))
							{
								any = true;
							}
							else
						...
						{
							if (Userform.checkValueByOperator(term.operator, Userform.strToDate(termValue), Userform.strToDate(value)))
							{
								any = true;
							}
							else
					...
					break;
				case "time":
					value = Userform.get24HourTime(term.field);
					...
					{
						if (Userform.checkValueByOperator(term.operator, termValue, value))
							any = true;
						else
							all = false;
					}
				...
				default:
					...
					if (Userform.checkValueByOperator(term.operator, term.value, value, term.field))
					{
						any = true;
					}
					else
		...
		if (condition.type == 'field')
		{
			...
			condition.action.each(function (action)
			{
				if (isConditionValid)
				{
					...
					{
						Userform.showField(action.field);
						Userform.restoreValue(action.field);
					}
					else
					{
						Userform.storeValue(action.field);
						Userform.hideField(action.field);
		...
		else if (condition.type == 'require')
		{
			...
			condition.action.each(function (action)
			{
				if (isConditionValid)
				{
					...
					{
						Userform.requireField(action.field, true);
					}
					else
					{
						Userform.requireField(action.field, false);
					}
				}
				else
				{
		...
		{
			var calcs = Userform.calculations;
			var cond = null;
			...
			if ((condition.link.toLowerCase() == 'any' && any) || (condition.link.toLowerCase() == 'all' && all))
			{
				...
				Userform.checkCalculation(calc);
			}
			else
			{
		...
		{
			Userform.log("any: %s, all: %s, link: %s", any, all, condition.link.toLowerCase());
			if (Userform.nextPage)
			{
				return;
			}
			if ((condition.link.toLowerCase() == 'any' && any) || (condition.link.toLowerCase() == 'all' && all))
			{
				...
				Userform.info('Correct: Skip To: ' + action.skipTo);
				var sections = $$('.form-section:not([id^=section_])');
				if (action.skipTo == 'end')
				{
					Userform.nextPage = sections[sections.length - 1];
				}
				else
				{
					Userform.nextPage = sections[parseInt(action.skipTo.replace('page-', ''), 10) - 1];
				}
			}
			else
			{
				Userform.info('Fail: Skip To: page-' + Userform.currentPage + 1);
				Userform.nextPage = false;
			}
		}
		Userform.enableDisableButtonsInMultiForms();
	},
	currentPage: false,
	nextPage: false,
	previousPage: false,
	...
	{
		if (!Userform.fieldConditions[field])
		{
			Userform.fieldConditions[field] = {
				event: event,
				conditions: []
			};
		}
		Userform.fieldConditions[field].conditions.push(condition);
	},
	widgetsAsCalculationOperands: [],
	unrequireField: function (qid) {
		if (!$('id_' + qid))
			return;
		$$('#id_' + qid + ' input, #id_' + qid + ' textarea, #id_' + qid + ' select').each(function (el) {
			...
			var corrected = Userform.corrected(el);
			if (validations.length > 0)
				el.addClassName('validate[' + validations.join(',') + ']');
		});
		if ($(qid + '-form-required')) {
			$(qid + '-form-required').remove();
		}
	},
	requireField: function (qid, req)
	{
		...
		$$('#id_' + qid + ' input, #id_' + qid + ' textarea, #id_' + qid + ' select').each(function (el)
		{
			// If an element is an internal stripe elements input, don't validate it.
			if (window.stripeWrapperClass && $j(el).closest(`.${stripeWrapperClass}`).length)
			{
				return true;
			}
			if (el.className.indexOf('no-validation') != -1)
			{
				return true;
			}
			var validations = [];
			...
			Userform.setFieldValidation(el);
		});
		if (req)
		{
			if ($('label_' + qid) && !$('label_' + qid).down('.form-required'))
			{
				//$('label_' + qid).insert('<span class="form-required">*</span>'); - this will add the star under the label text SZ CWNP-648		
				if ($$('#label_' + qid + ' label').length == 0)
					$('label_' + qid).insert("<label />");
				$$('#label_' + qid + ' label').first().insert({ after: new Element('span', { class: 'form-required', id: qid + '-form-required' }).update('*') });
			}
		}
		else
		{
	...
	{
		if (Userform.widgetsAsCalculationOperands.include(id))
		{
			if (document.createEvent)
			{
				var evt = document.createEvent('HTMLEvents');
	...
	{
		$A(Userform.calculations).each(function (calc, index)
		{
	...
	setCalculationEvents: function ()
	{
		...
		$A(Userform.calculations).each(function (calc, index)
		{
			...
			for (var i = 0; i < ops.length; i++)
			{
				...
				var type = Userform.getInputType(opField),
					ev;
				switch (type)
				{
				case "widget":
					setCalculationListener($('id_' + opField), 'change', calc);
					Userform.widgetsAsCalculationOperands.push(opField);
					break;
				case 'radio':
				case 'checkbox':
	...
	{
		Userform.calcValues[id] = calculationValues.split('|');
	},
	...
	checkCalculation: function (calc)
	{
		...
		var getValue = function (data, numeric)
		{
			...
			var type = Userform.getInputType(data);
			var val = '';
			switch (type)
			{
			case 'radio':
				$$("#id_" + data + ' input[type="radio"]').each(function (rad, i)
				{
					if (rad.checked)
					{
						if (Userform.calcValues[data] && i < Userform.calcValues[data].length)
						{
							val = Userform.calcValues[data][i];
						}
						else
						{
							val = rad.value;
			...
			case 'checkbox':
				...
				$$("#id_" + data + ' input[type="checkbox"]').each(function (chk, i)
				{
					if (chk.checked)
					{
						if (Userform.calcValues[data] && i < Userform.calcValues[data].length)
						{
							valArr.push(Userform.calcValues[data][i]);
						}
						else
						{
				...
				break;
			case 'select':
				if (Userform.calcValues[data] && $('input_' + data).selectedIndex < Userform.calcValues[data].length)
				{
					val = Userform.calcValues[data][$('input_' + data).selectedIndex];
				}
				else
				{
		...
		if (parseFloat(output) === 0 && Userform.defaultValues['input_' + result])
		{
			output = Userform.defaultValues['input_' + result];
		}
	...
	{
		if (Userform.widgetsWithConditions.include(id))
		{
			if (document.createEvent)
			{
				var evt = document.createEvent('HTMLEvents');
	...
	setConditionEvents: function ()
	{
		try
		{
			$A(Userform.conditions).each(function (condition)
			{
				...
				if (condition.type == 'field' || condition.type == 'calculation' || condition.type == 'require')
				{
					...
					{
						var id = term.field;
						switch (Userform.getInputType(id))
						{
						case "widget":
							Userform.setFieldConditions('input_' + id, 'change', condition);
							Userform.widgetsWithConditions.push(id);
							break;
						case "combined":
							Userform.setFieldConditions('id_' + id, 'keyup', condition);
							break;
						case "address":
							// address might be a part of pay form so the condition check should also be triggered when the payment method changes
							Userform.setFieldConditions('id_' + id, 'input', condition);
							Userform.setFieldConditions('id_' + id, 'keyup', condition);
							Userform.setFieldConditions('input_' + term.field + '_country', 'change', condition);
							break;
						case "datetime":
							Userform.setFieldConditions('id_' + id, 'date:changed', condition);
							break;
						case "birthdate":
							Userform.setFieldConditions('input_' + id + '_day', 'change', condition);
							Userform.setFieldConditions('input_' + id + '_month', 'change', condition);
							Userform.setFieldConditions('input_' + id + '_year', 'change', condition);
							break;
						case "time":
							Userform.setFieldConditions('input_' + id + '_hourSelect', 'change', condition);
							Userform.setFieldConditions('input_' + id + '_minuteSelect', 'change', condition);
							Userform.setFieldConditions('input_' + id + '_ampm', 'change', condition);
						case "select":
						case "file":
							Userform.setFieldConditions('input_' + id, 'change', condition);
							break;
						case "checkbox":
						case "radio":
							Userform.setFieldConditions('id_' + id, 'click', condition);
							break;
						case "number":
							Userform.setFieldConditions('input_' + id, 'number', condition);
							break;
						case "autocomplete":
							Userform.setFieldConditions('input_' + id, 'autocomplete', condition);
							break;
						case "grading":
							Userform.setFieldConditions('id_' + id, 'keyup', condition);
							break;
						default:
							Userform.setFieldConditions('input_' + id, 'keyup', condition);
						}
					});
				}
				else
				{
					$A(condition.terms).each(function (term)
					{
						...
						var nextButton = Userform.getSection($('id_' + id)).select('.form-pagebreak-next')[0];
						if (!nextButton)
						{
							return;
						}
						...
						{
							Userform.checkCondition(condition);
						});
					});
				}
			});
	...
	handlePaymentSubProducts: function ()
	{
		...
		function showSubProducts(el)
		{
			...
			setTimeout(Userform.totalCounter(Userform.prices), 300);
		};
	},
	countTotal: function (prices)
	{
		var prices = prices || Userform.prices;
		var discounted = false;
		if (Object.keys(Userform.discounts).length > 0)
		{
			discounted = true;
			if (Userform.discounts.total || Userform.discounts.shipping)
			{
				var type = Userform.discounts.type,
					rate = Userform.discounts.rate,
					minimum = Userform.discounts.minimum,
					code = Userform.discounts.code;
			}
			else
			{
				for (var pid in prices)
				{
					for (var kkey in Userform.discounts)
					{
						if (pid.indexOf(kkey) !== -1)
						{
							prices[pid].discount = Userform.discounts[kkey];
						}
					}
				}
			}
		...
		$H(prices).each(function (pair)
		{
			...
			if (pair.value.tax)
			{
				...
				{
					var selectedIndex = $$('select[id*="input_' + tax.surcharge.field + '"]')[0].getSelected().index - 1;
		...
		if (Userform.discounts.total)
		{
			...
			{
				'top': Userform.discounts.container
			});
			$('discount_total').update(parseFloat(reduce).toFixed(2));
		}
		if (Userform.discounts.shipping && shippingTotal > 0 && subTotal >= minimum)
		{
	...
	totalCounter: function (prices)
	{
		...
		Userform.prices = prices;
		document.observe('dom:loaded', Userform.countTotal(prices));
		$H(prices).each(function (pair)
		{
			$(pair.key).observe('click', function ()
			{
				Userform.countTotal(prices);
			});
			if (pair.value.tax)
			{
			...
			if ($(pair.value.quantityField))
			{
				...
				{
					if (Userform.isVisible($(pair.value.quantityField)))
					{
			...
			if ($(pair.value.specialPriceField))
			{
				...
				{
					if (Userform.isVisible($(pair.value.specialPriceField)))
					{
		...
		if ($('coupon-button'))
		{
			...
			cb.observe('click', function ()
			{
				if (ci.value)
				{
					...
					cb.disable();
					var a = new Ajax.Jsonp(Userform.server,
					{
						parameters:
						{
							action: 'checkCoupon',
						...
						onComplete: function (t)
						{
							...
							if (t.success)
							{
								...
								else
								{
									...
									Userform.applyCoupon(t.message);
								}
							}
						}
					});
	...
	applyCoupon: function (discount)
	{
		...
		cb.observe('click', function ()
		{
			...
			if (Userform.discounts.container)
			{
				Userform.discounts.container.remove();
			}
		...
		if (!discount.paymentType || (discount.paymentType && discount.paymentType === "product"))
		{
			if (discount.apply === "product")
			{
				...
				{
					Userform.discounts[pid] = discount.rate + '-' + discount.type;
					$$('span[id*="_price"]').each(function (field, id)
					{
		...
		else
		{
			$A(discount.products).each(function (pid)
			{
				...
				{
					Userform.discounts[pid] += "-all";
				}
				else
				{
					Userform.discounts[pid] += "-first";
				}
				$$('span[id*="_price"]').each(function (field, id)
				{
	...
	// SITES-1716: This method sets up the old captcha. It caused an error. Now that the old captcha isn't
	// loaded into the page anyway, it's simplest just to drop the method body.
	...
	{
		var date = d.date;
		var month = Userform.addZeros(date.getMonth() + 1, 2);
		var day = Userform.addZeros(date.getDate(), 2);
	...
	highLightLines: function ()
	{
		$$('.form-line').each(function (l, i)
		{
			l.select('input, select, textarea, div, table div, button').each(function (i)
			{
				...
				{
					if (Userform.isCollapsed(l))
					{
						Userform.getCollapseBar(l).run('click');
					}
					if (!Userform.highlightInputs)
					{
						return;
					}
	...
	getForm: function (element)
	{
		...
		return Userform.getForm(element.parentNode);
	},
	getContainer: function (element)
	{
		...
		if (element.hasClassName("form-line"))
		{
			...
			if (typeof(returnValue.fieldElements) == "undefined")
				returnValue.fieldElements = [];

			return returnValue;
		}
		return Userform.getContainer(element.parentNode);
	},
	getSection: function (element)
	{
		...
		return Userform.getSection(element.parentNode);
	},
	getCollapseBar: function (element)
	{
		...
		return Userform.getCollapseBar(element.parentNode);
	},
	isCollapsed: function (element)
	{
		...
		return Userform.isCollapsed(element.parentNode);
	},
	isVisible: function (element)
	{
		element = $(element);
		...
		if (element.hasClassName("form-textarea") && element.up('div').down('.nicEdit-main') && (element.up('.form-line') && Userform.isVisible(element.up('.form-line'))))
		{
			return true;
		}
		if (element.style.display == "none" || element.style.visibility == "hidden")
		...
		return Userform.isVisible(element.parentNode);
	},
	...
	setButtonActions: function ()
	{
				...
				b.observe('click', function ()
				{
					...
					{						
						if (!$$('.form-error-message')[0] && !$$('.form-textarea-limit-indicator-error')[0])
						{
							...
							{
								bu.innerHTML = Userform.texts.pleaseWait;
		...
		$$('.form-submit-reset').each(function (b)
		{
			b.onclick = function ()
			{
				if (!confirm(Userform.texts.confirmClearForm))
				{
					return false;
				}
		...
		$$('.form-submit-print').each(function (print_button)
		{
			print_button.observe("click", function ()
			{
				...
				$$('.form-textarea, .form-textbox').each(function (el)
				{
					...
					//we're removing date-separate from generated forms, but older generated forms might still have it
					var dateSeparate;
	...
	hasHiddenValidationConflicts: function (input)
	{
		...
		return hiddenOBJ && (hiddenOBJ.hasClassName('form-field-hidden') || (hiddenOBJ.up('ul.form-section') && hiddenOBJ.up('ul.form-section').hasClassName('form-field-hidden')));
	},
	initGradingInputs: function ()
	{
		...
		$$('.form-grading-input').each(function (item)
		{
			...
			item.validateGradingInputs = function ()
			{
				...
				if (_this.hasHiddenValidationConflicts(item)) return Userform.corrected(item);
				if (isNotNumeric)
				{
					return Userform.errored(item, Userform.texts.numeric);
				}
				if ($("grade_total_" + id))
				{
					...
					{
						$("grade_error_" + id).innerHTML = ' ' + Userform.texts.lessThan + ' <b>' + allowed_total + '</b>.';
						return Userform.errored(item, Userform.texts.gradingScoreError + " " + allowed_total);
					}
					else
					{
						return Userform.corrected(item);
					}
				}
				else
				{
	...
	initSpinnerInputs: function ()
	{
		...
		$$('.form-spinner-input').each(function (item)
		{
			...
			item.validateSpinnerInputs = function ()
			{
				...
				{
					return Userform.errored(item, Userform.texts.inputCarretErrorA + " " + min_val);
				}
				else if (max_val && userInput > max_val)
				{
					return Userform.errored(item, Userform.texts.inputCarretErrorB + " " + max_val);
				}
				else
				{
					return Userform.corrected(item);
	...
	initCreditCardInputs : function()
	{
		...
		const visaRegex = /^4[0-9]{1}/;
		const isracardRegex = /^(37|4[0-589]|5[1-5]|60)/;

		const acceptedCards = new Set();

		if (payformSettings.exists()) {
			var typeArray = payformSettings._options.creditCard.type;
			for (let i = 0; i < typeArray.length; i++) {
				if (typeArray[i].enabled) {
					acceptedCards.add(typeArray[i].name);
				}
			}
		}

		$$('.form-creditcard').each(function (item)
		{
			var noValidationRequired = formJson[_this.getContainer(item).id.split("_")[1] + "_required"] == "No";
			item.observe('input', function ()
			{
				item.validateCreditCardInputs();
			});
			item.validateCreditCardInputs = function()
			{
				var item = this;
				var isIsracard = false;
				var wrongCardType = false;
				var errored = false;

				if (payformSettings.exists())
				{
					var ccType = $$('#input_' + payformSettings.id() + '_cc_type')[0];
					isIsracard = ccType && ccType.value.toLowerCase() == 'isracard';

					// If this is a new version of forms which has a hidden credit card type input then run the detect method.
					if ($j('#input_' + payformSettings.id() + '_cc_type').prop("type") == 'hidden') {
						wrongCardType = !item.detectCreditCardType();
					}
				}				

				if (noValidationRequired && item.value == "")
					return Userform.corrected(item);

				item.errored = false;
				if (!Userform.isVisible(item))
					return Userform.corrected(item);
				...
				if (isIsracard) {
					errored = !Co.Forms.Validation.IsValidIsraCard(value);
				}
				else {
					errored = !Co.Forms.Validation.ValidateCardType(ccType, value, ccType.tagName.toLowerCase());

					if (!errored)
					{
						var cardNumberLength = value.length, digitCounter, numberProduct, checkSumTotal = 0, productDigitCounter;
						...
						for (digitCounter = cardNumberLength - 1; digitCounter >= 0; digitCounter--)
						{
							...
							for (productDigitCounter = 0; productDigitCounter < numberProduct.length; productDigitCounter++)
								checkSumTotal += parseInt(numberProduct.charAt(productDigitCounter));
						}

						errored = (checkSumTotal % 10 != 0);
					}
				}

				if (wrongCardType)
					return Userform.errored(item, Userform.texts.invalidCardTypeError);
				else if (errored)
					return Userform.errored(item, Userform.texts.creditCardError);
				else
					item.value = value;
				return Userform.corrected(item);
			};
			item.detectCreditCardType = function () {
				var detectedCard;


				// Our regex for Isracard might not catch all Isracards.
				// Furthermore, some Isracards cards might start with the same digits as Amex or other cards.
				// We therefore do a special check for Isracard before all the others to garauntee that a fully entered Isracard will 
				// be properly marked as such.
				if (acceptedCards.has("Isracard") && Co.Forms.Validation.IsValidIsraCard(item.value.trim())) {
					detectedCard = "Isracard";
				} else if (discoverRegex.test(item.value)) {
					detectedCard = "Discover";
				} else if (amexRegex.test(item.value)) {
					detectedCard = "Amex";
				} else if (mastercardRegex.test(item.value)) {
					detectedCard = "Mastercard";
				} else if (visaRegex.test(item.value)) {
					detectedCard = "Visa";
				} else if (isracardRegex.test(item.value)) {
					detectedCard = "Isracard";
				} else {
					detectedCard = "Unknown";
				}

				$j(".cc-field-wrapper").removeClass("icon visa-icon amex-icon discover-icon mastercard-icon isracard-icon");

				if (acceptedCards.has(detectedCard)) {
					$j('#input_' + payformSettings.id() + '_cc_type').val(detectedCard);
					var iconClass = detectedCard.toLowerCase() + "-icon";
					$j(".cc-field-wrapper").addClass("icon " + iconClass);
				}
				// If the user has put in 2 numbers and the type is not a match, we treat it as an invalid card type error as opposed to an incomplete field.
	...
	initNumberInputs: function ()
	{
		...
		$$('.form-number-input').each(function (item)
		{
			...
			item.validateNumberInputs = function ()
			{
				...
				{
					return Userform.errored(item, Userform.texts.maxDigitsError + " " + max_len);
				}
	...
	handlePages: function ()
	{
		...
		$$('.form-pagebreak').each(function (page, i)
		{
			...
			{
				Userform.currentSection = section;
			}
			pages.push(section);
			...
			section.select('.form-pagebreak-next').invoke('observe', 'click', function ()
			{
				...
				var form = Userform.getForm( section );
				if( Userform.validateAll( form ) )
				{
					if (window.parent && window.parent != window)
					{
					...
					if (Userform.nextPage)
					{
						Userform.backStack.push(section.hide());
						Userform.currentSection = Userform.nextPage.show();
						if (!$this.noJump)
						{
							Userform.currentSection.scrollIntoView(true);
						}
						Userform.enableDisableButtonsInMultiForms();
					}
					...
					{
						Userform.backStack.push(section.hide());
						Userform.currentSection = section.next().show();
						Userform.currentSection.select('.form-line').each(function (el)
						{
							var id = el.id.split("_")[1];
							if (Userform.getInputType(id) === 'widget')
							{
								Userform.showWidget(id);
							}
						});
						if (!$this.noJump)
						{
					...
					Userform.nextPage = false;
					if (Userform.saveForm)
					{
						Userform.hiddenSubmit(Userform.getForm(section));
					}
				}
				else
				{
					try
					{
						...
						$$('.form-pagebreak-next').each(function (nextButton)
						{
							...
							errorBox.insert(Userform.texts.generalPageError);
			...
			section.select('.form-pagebreak-back').invoke('observe', 'click', function ()
			{
				...
				section.hide();
				Userform.currentSection = Userform.backStack.pop().show();
				if (!$this.noJump)
				{
					Userform.currentSection.scrollIntoView(true);
				}
		...
		if ($('creditCardTable'))
		{
			$$('.paymentTypeRadios').each(function (radio)
			{
				radio.observe('click', function ()
				{
					...
					if (radio.checked && radio.value === "credit" && (Userform.paymentTotal > 0 || Object.keys(Userform.discounts).length === 0))
					{
						$('creditCardTable').show();
					}
				});
	...
	description: function (input, message)
	{
		...
		var cont = Userform.getContainer(input);
		if (!cont)
		{
			return;
		}
		...
		if ("desc" in document.get && document.get.desc == 'v2')
		{
			...
			bubble.addClassName('form-right');
		}
	...
	validateAll: function (form)
	{
				...
				if (jQuery(pad).attr("data-required") === "true")
				{
					if (jQuery(pad).parent().parent().parent().is(":visible"))
					{
						...
						if (jQuery(pad).jSignature('getData', 'base30')[1].length == 0)
						{
							...
							{
								w.append('<div class="form-line-error" style="float:left;">' + '<div class="form-error-message">' + '<img src="/images/sitecontrol/formbuilder/exclamation-octagon.png" align="left" style="margin-right:5px;">' + '<div class="form-error-arrow">' + '<div class="form-error-arrow-inner"></div>' + '</div>' + 'This field is required</div></div>');
							}
						}
						else
						{
		...
		if (window.JCFServerCommon !== undefined)
		{
			...
			{
				if (Userform.isVisible(el))
				{
					if ($(Userform.currentSection) && $(Userform.currentSection).select('.form-section').length > 0)
					{
						if (el.up('.form-section').id === $(Userform.currentSection).select('.form-section')[0].id)
						{
		...
		$$(c + '*[class*="validate"]').each(function (input)
		{
			...
			{
				ret = Userform.hasHiddenValidationConflicts(input);
				_log('ret setted ' + ret);
			}
		});
		//seems this was put here for textarea validation but it's also 
		$$(/*c + ' input.form-validation-error, '*/ + c + ' textarea.form-validation-error').each(function(input) {
			_log('looping inputs with validation :');
			_log(input);
			if (input.validateValue === undefined)
			{
				_log('no required continuing');
				return;
			}
			if (!(!!input.validateValue && input.validateValue()))
			{
				ret = Userform.hasHiddenValidationConflicts(input);
				_log('ret setted ' + ret);
			}
		...
		var firstErrorElement = document.getElementsByClassName('form-validation-error')[0];
	...
	{
		// This catches any input which consists of multiple inputs. For example, phone (area code and rest) and more.
		var errorIsOnInputGroup = $j(input).parents('.form-input').find('input').length > 0;
		...
		{
			var preLink = "http://chabad.org/";
		}
		else
		{
			var preLink = "chabad.org/";
		}
		if (Userform.isCollapsed(input))
		{
			var collapse = Userform.getCollapseBar(input);
			if (!collapse.errored)
			{
				collapse.select(".form-collapse-mid")[0].insert(
				{
					top: '<img src="' + '/images/sitecontrol/formbuilder/exclamation-octagon.png" align="bottom" style="margin-right:5px;"> '
				}).setStyle(
				{
					color: 'red'
				});
		...
		var container = Userform.getContainer(input);

		if (!container.fieldElements.include(input))
			container.fieldElements.push(input);
					
		input.errored = true;
		input.addClassName('form-validation-error');
		
		if (!errorIsOnInputGroup) {
		...
		// We don't have a way to show an erorr message on a particular form input in a form group (e.g payment details), so we
		// show the error as being for the entire group. In such a case, we switch the message to be more general (something is missing) instead
		// of "*this* field is required", because there's no one "this field".
		if (errorIsOnInputGroup && message == this.texts.required) {
			message = this.texts.requiredGroupInput;
		}

		insertEl.insert(new Element('div',
		{
			className: 'form-error-message'
		}).insert('<i class="fa fa-fw fa-exclamation-circle"></i>&nbsp; ' + message).insert(new Element('div',
		{
	...
	corrected: function (input)
	{
		...
		var hasError = false;
		if (container.validating)
			return true;
		
		container.validating = true;
		var fields = container.fieldElements;
		for (var i = fields.length-1; i >= 0; i--)
		{
			if (fields[i].errored && fields[i].validateInput)
			{
				if (!fields[i].validateInput())
				{
					container.validating = false;
					return false;
				}
				break;
			}
	...
	// Clears client side validation errors after they have been corrected. Errors which are added by the server should 
	// only be removed by the server, because only it can know that the issue no longer occurs. For example, if POST response
	// gives us an error that the payment processor is down, we won't know if it's fixed until we POST.
	hideErrorMessage: function ()
	{
		$$('.form-error-js').invoke('remove');
	},
	showErrorMessage: function ()
	{
		...
		var errorBox = new Element('div',
		{
			// Add an identifier for client side errors, such as missing required fields. This enables us to
			// remove client side errors, while keeping server side errors (eg from a previous POST) visible.
			class: 'form-error-js',			
		});
		errorBox.insert(Userform.texts.generalError);
		$('formContainer').insert({top:errorBox});
		
	},
	setupStripeElements: function () {
		if (!('hasStripeConfiguration' in window)) {
			return;
		}

		$A(Userform.forms).each(function (form) {
			if (!hasStripeConfiguration(form)) {
				return;
			}

			let creditCardNumberInput = form.querySelector('.js-cc-number');
			
			if (!creditCardNumberInput) {
				return;
			}

			let cvvInput = form.querySelector('input[id$=cc_ccv]');
			
			let expirationLabelWrapper = $j(
				`<td><span class="form-sub-label-container">
					<div class="form-textbox js-stripe-mount-point"></div>
					<label class="form-sub-label">${"Expiration Date (MM/YY)".Translate()}</label>
				</span></td>`);

			let expirationInputsParent = $j(form).find('.credit_card [name*=cc_exp_month]').closest('tr');
			expirationInputsParent.empty();
			expirationInputsParent.append(expirationLabelWrapper);

			let expirationMountPoint = expirationLabelWrapper.get(0).querySelector('.js-stripe-mount-point');

			setupStripe(form, creditCardNumberInput, cvvInput, expirationMountPoint);
		});
	},
	validator: function ()
	{
		...
		$A(Userform.forms).each(function (form)
		{
			...
			$j(form).on('submit', function (e)
			{
				try
				{
					...
					if (!Userform.validateAll(form)){
						Userform.enableButtons();
						Userform.showErrorMessage();
						form.Submitted = false;
						Event.stop(e);
						return;
					}

					// SITES-2256: Don't try to process stripe if not making a credit card payment.
					var paymentMethods = $$('.paymentMethod') || [];
					var activePaymentMethod = paymentMethods.find(function(method) { return method.checked });
					// SITES-2297: Only require stripe payment method to be "selected" if there are other payment methods to select.
					var isCreditCardOfMultiple = activePaymentMethod && activePaymentMethod.value === 'creditCard';
					var isCreditCard = isCreditCardOfMultiple || !paymentMethods.length;
					var shouldProcessCreditCardPayment = isCreditCard && (payformSettings.isAlwaysRequired() || grandTotal > 0);
					var stripeElementsValidationState = shouldProcessCreditCardPayment && 'validateStripeElements' in window && validateStripeElements(form);
					// SITES-2351: If validation passed even though credit card number is empty, apperantly it isn't required or being used.
					// If stripe elements is valid, they have the credit card number, not us, but it's still valid.
					var isFilled = !!$j('.js-cc-number').val() || stripeElementsValidationState?.valid;
					
					if (stripeElementsValidationState?.error) {
						Userform.enableButtons();
						Co.Dialog.Alert(stripeElementsValidationState.error);
						Event.stop(e);
						return;
					}

					// SITES-2560: validate recaptcha before processing stripe payment intent
					if (!Co.Forms.Validation.CheckCaptcha (form)) {
						Event.stop(e);
						return;
					}

					if (!Co.Forms.Validation.CheckVisibleCaptcha(form)) {
						Userform.enableButtons();
						Co.Dialog.Alert('Please check the recaptcha box.'.Translate().valueOf());
						Event.stop(e);
						return;
					}
					
					// Stripe integration is only loaded on pages which need it.
					if (isFilled && isCreditCard && 'hasStripeProcessing' in window && hasStripeProcessing(form)) {
						// Stop submit until stripe is done.
						Event.stop(e);

						processWithStripe(form, {
							submissionIdInputName: 'source-submission-id',
							useConnect: true
						}).then(function () {
							$j(form).trigger('submit');
						}).catch(function (ex) {
							Userform.enableButtons();
							var errorMessage = 'An error occurred while processing your payment. Please try again, or contact us for assistance.'.Translate();

							if (ex) {
								errorMessage += "<br>" + 'Additional information:'.Translate() + ' ' + ex;
							}
							// We use valueOf because Alert must receive a string primitive but Translate returns a string object.
							Co.Dialog.Alert(errorMessage.valueOf());
						});
					}
				}
				catch (err)
				...
				if (Userform.compact && Userform.imageSaved == false)
				{
					Event.stop(e);
			...
			{
				Userform.setFieldValidation(input);
			});
			$$('input, textarea').each(function(input) {
				//console.log(input);
				Userform.validateInputValue(input);
			});
			$$('.form-upload').each(function (upload)
			{
				try
				{
					...
					const handleMissingFile = (uploadElement) => {
						if (isRequired(uploadElement))
						{
							Userform.errored(upload, 'This field requires a file attachment.');
							clearErrorOnFileInput(uploadElement);
						}
					}

					const clearErrorOnFileInput = (uploadElement) => {
						const isCustomUploadComponent = uploadElement.tagName.toLowerCase() === 'magen-file-drop-zone';
						const eventName = isCustomUploadComponent ? 'dropzone-files-changed' : 'input';
						const eventHandler = () => {
							if (uploadElement.files?.length)
							{
								Userform.corrected(uploadElement);
								uploadElement.removeEventListener(eventName, eventHandler);
							}
						};

						// remove error once a file is added
						uploadElement.addEventListener(eventName, eventHandler);
					}
					...
					upload.validateInput = function ()
					{
						...
						if (exVal() !== false)
						{
							...
							if (!upload.files)
							{
								handleMissingFile(upload);
								return !isRequired(upload);
							}
							var acceptString = upload.readAttribute('accept') || Userform.uploadAllowedExtensions.join(', ') || "";
							...
							var ext = "";
							if (Userform.getFileExtension(file.fileName))
							{
								ext = Userform.getFileExtension(file.fileName);
							}
							
							const isExtensionIncluded = (extension) => accept.some(ex => ex.toLowerCase().contains(extension.toLowerCase()));
							if (acceptString != "*" && !isExtensionIncluded(ext))
							{
								return Userform.errored(upload, Userform.texts.uploadExtensions + '<br/>' + acceptString.replace(/, ?/g, ', '));
							}
							var validateImage = upload.readAttribute('data-imagevalidate') || false;
							var validatedImageExt = "jpeg, jpg, png, gif, bmp";
							if (validateImage && (validateImage === 'yes' || validateImage === 'true') || (validatedImageExt.include(ext) || validatedImageExt.include(ext.toLowerCase())))
							{
								if (!file.type.match("image"))
								{
									return Userform.errored(upload, "You have uploaded an invalid image file type.");
								}
							}
							if (!file.fileSize)
							{
							...
							if (file.fileSize > Userform.uploadMaxSize)
							{
								return Userform.errored(upload, Userform.texts.uploadFilesize + ' ' + formatSize(Userform.uploadMaxSize, 0));
							}
							return Userform.corrected(upload);
						}
					};
					if (!required)
					{
	...
	{		
		...
		input.validateValue = function()
		{
			...
			if (!Userform.isVisible(input))
			{
				if (input.errored && validations.include("visible"))
					return Userform.corrected(input);

				return true;
			}
			if (validations.without("visible").length > 0 && input.hasClassName("form-validation-error"))
				return false;
				
			if (!$(input.parentNode).hasClassName('form-matrix-values') && !validations.include("paymentMethod"))
				Userform.corrected(input);
			if (reg.tags.test(input.value))
				return Userform.errored(input, Userform.texts.html);				
	...
	setFieldValidation: function (input)
	{
		...
		input.validateInput = function (deep)
		{
			...
			//reset validation in case validation changed based on user action - i.e. require payform based on amount CWNP-648
			validations = input.className.replace(/.*validate\[(.*)\].*/, '$1').split(/\s*,\s*/);
			...
			if (vals.include('disallowFree'))
			{
				...
				for (var i = 0; i < freeEmails.length; i++)
				{
					...
					{
						return Userform.errored(input, Userform.texts.freeEmailError);
					}
				}
			}
			if (vals.include('minSelection'))
			{
				...
				{
					return Userform.errored(input, Userform.texts.minSelectionsError + minSelection + '.');
				}
			}
			if (vals.include('maxSelection'))
			{
				...
				{
					return Userform.errored(input, Userform.texts.maxSelectionsError + maxSelection + '.');
				}
			}
			if (vals.include('disallowPast'))
			{
				var id = input.id.split('_').last();
				var inputtedDate = Userform.getDateValue(id).split('T')[0];
				var dat = new Date();
				...
				if (Userform.checkValueByOperator('before', Userform.strToDate(currentDate), Userform.strToDate(inputtedDate)))
				{
					return Userform.errored(input, Userform.texts.pastDatesDisallowed);
				}
			}
			if (vals.include("creditcard"))
			{				
				return input.validateCreditCardInputs();
			}
			...
			if (vals.include("Email_Confirm"))
			{
				...
				{
					return Userform.errored(input, Userform.texts.confirmEmail);
				}
				else if (($('input_' + idEmail + '_confirm').value) && (!reg.email.test($('input_' + idEmail + '_confirm').value)))
				{
					return Userform.errored(input, Userform.texts.email);
				}
			}
			if (vals.include("paymentMethod") && vals.include("required")) // validate payment method -ML
			{
				var paymentMethods = $$('.paymentMethod'), errored = false, paymentSelected = false;
				
				for (var i = 0; i < paymentMethods.length; i++)
				{
					if (paymentMethods[i].checked && !paymentMethods[i].disabled)
					{
						paymentSelected = true;
						break;
					}
					else if (paymentMethods[i].errored)
						errored = true;
				}
				...
				if (!paymentSelected)
				{
					if (errored)
						return false;
					return Userform.errored(input, Userform.texts.paymentMethod);
				}
				else
					Userform.corrected(input);
			}
			
			if(vals.include("other"))
			{
				Userform.corrected(input);				
				var qid = input.id.split('_')[1];
				if (!isNaN(qid))
				{
					var allowOther = formJson[qid + '_allowOther'] == 'Yes';
					if (allowOther)
					{
						if (document.getElementById('other_' + qid).checked)
						{
							if (document.getElementById('input_' + qid).value.trim() == '')
								return Userform.errored(input, 'Please enter a value'.locale());							
						}
					}
				}
			}

			/*if (vals.include("customPartial"))
			{
				Userform.corrected(input);
				if (!parseFloat(document.getElementById('input_partial').value))
				{
					return Userform.errored(input, 'Please enter a valid number'.locale());
				}

			}*/						
			if (vals.include("customPartial")) //validate partial payment -ML
			{
				
				Userform.corrected(input);

				if (document.getElementById('other_partial').checked)
				{
					for (var key in formJson)
					{
						if (key.split('_')[1] === 'type' && formJson[key] === 'control_totalamount')
						{
							var options = key.split('_')[0] + '_';
							var partialType = formJson[options + 'partialPayType'];
							var percent = formJson[options+'partialPayMinimum'];
							break;
						}
					}
					var min=0;
					if (partialType === 'percent')
						min=grandTotal*percent/100;
					else
						min=percent;
					if (!parseFloat(document.getElementById('input_partial').value))
					{
					...
					if (parseFloat(document.getElementById('input_partial').value) < min)
					{
						return Userform.errored(input, Userform.texts.paymentMinimum
							.replace('{amount}', formatCurrency(document.getElementById('input_partial').value))
							.replace('{minimum}', formatCurrency(min))
							);
					}

				}
			}
			if (vals.include("required") || (vals.include("paymentMethod") && grandTotal > 0))
			{
				if (input.tagName == 'INPUT' && input.readAttribute('type') == "file")
				{
					...
					{
						return Userform.errored(input, Userform.texts.required);
					}
					else
					{
						return Userform.corrected(input);
				...
				else if (input.tagName == "INPUT" && (input.readAttribute('type') == "radio" || input.readAttribute('type') == "checkbox"))
				{
					...
					if ($(input.parentNode).hasClassName('form-matrix-values'))
					{
						...
						{
							if (!oneChecked)
								return Userform.errored(input, Userform.texts.requireOne);
						}
						else if (!$H(matrixRows).values().all())
						{
							return Userform.errored(input, Userform.texts.requireEveryRow);
						}
						else
						{
							return Userform.corrected(input);
				...
				else if (input.name && input.name.include("["))
				{
					...
					try
					{
						...
						var checkValues = cont.select('input,select[name*=' + input.name.replace(/\[.*$/, '') + ']').map(function (e)
						{
							...
							//if the input is hidden do not validate --mshanowitz
						...
						var innerVals = e.className.replace(/.*validate\[(.*)\].*/, '$1').split(/\s*,\s*/);
							...
							if (innerVals.include('required') && Userform.isVisible(e))
							{
								var validateField = true;								
								if(innerVals.include('other'))
								{
									switch(e.type)
									{
										case 'text':											
											if(!isNaN(e.id.split('_')[1]))										
												validateField = $('other_' + e.id.split('_')[1]).checked;										
											break;
										case 'radio':
										case 'checkbox':
												validateField = false;
							...
							// We only fail the currently checked input based on a sibling if the very complicated bit of code which
							// creates this array tells us to.
							// Without the filter, related inputs always fail, because they always have siblings. --SITES-1215
				...
				//Added that it should not validate if hidden --mshanowitz
				if (((!input.value || input.value.strip(" ").empty() || input.value.replace('<br>', '').empty() || input.value == 'Please Select') && !(input.readAttribute('type') == "radio" || input.readAttribute('type') == "checkbox") && !$(input.parentNode).hasClassName('form-matrix-values')) && input.offsetParent !== null )
				{
					var innerVals = input.className.replace( /.*validate\[(.*)\].*/ , '$1').split( /\s*,\s*/ );
					var validateField = true;
					if (innerVals.include('other'))
					{
						switch (input.type)
						{
							case 'text':
								if (!isNaN(input.id.split('_')[1]))
									validateField = $('other_' + input.id.split('_')[1]).checked;
								break;
							case 'radio':
								validateField = false;
								break;
			...
			case "Alphabetic":
				...
				{
					return Userform.errored(input, Userform.texts.alphabetic);
				}
				break;
			case "Numeric":
				if (!reg.numeric.test(input.value) && !reg.numericDotStart.test(input.value))
				{
					return Userform.errored(input, Userform.texts.numeric);
				}
				break;
			case "AlphaNumeric":
				if (!reg.alphanumeric.test(input.value))
				{
					return Userform.errored(input, Userform.texts.alphanumeric);
				}
				break;
			case "Url":
				if (!reg.url.test(input.value))
				{
					return Userform.errored(input, Userform.texts.url);
				}
				break;
			default:
			}
		...
		var validatorEvent = function (e)
		{
			setTimeout(function ()
			{
				...
				{
					input.validateInput();
					//if(!$$('.form-error-message').any())
						//Userform.hideErrorMessage();
				}
				else if (input.type == "hidden" || input.type == 'file' || input.type == 'radio' || input.type == 'checkbox')
				{
					input.validateInput();
					//if(!$$('.form-error-message').any())
						//Userform.hideErrorMessage();
		...
		{
			input.observe('click', validatorEvent);
		}
		else
		{
			input.observe('blur', validatorEvent);
		...
		/*
		if (input.id === "other_partial") // validate partial amount -ML
		{
			for (var key in formJson)
			{
				if (key.split('_')[1] === 'type' && formJson[key] === 'control_payform')
				{
					var options = formJson[key.split('_')[0] + '_options'];
					var partialType = options.partialPay.type;
					var percent = options.partialPay.amount;
					break;
				}
			}
			var min=0;
			...
			{
				return Userform.errored(document.getElementById('other_partial'), Userform.texts.paymentMinimum
																				.replace('{amount}', formCurrency.symbol() + parseFloat(input.value).toFixed(2))
																				.replace('{minimum}', formCurrency.symbol() + parseFloat(min).toFixed(2))
																				);
			}
		}*/
	...
	FBInit: function ()
	{
		Userform.FBNoSubmit = true;
		FB.getLoginStatus(function (response)
		{
			if (response.authResponse)
			{
				Userform.FBCollectInformation(response.authResponse.userID);
			}
			else
			{
	...
	FBCollectInformation: function (id)
	{
		...
		try
		{
			...
			query.wait(function (rows)
			{
				...
				Userform.bringOldFBSubmissionBack(id);
				var hidden = new Element('input',
				{
					type: 'hidden',
					name: 'fb_user_id'
				}).setValue(id);
				var form = Userform.getForm(inp);
				form.insert(
				{
					top: hidden
				});
	...
	bringOldFBSubmissionBack: function (id)
	{
		...
		var a = new Ajax.Jsonp(Userform.url + 'server.php',
		{
			...
			onComplete: function (t)
			{
				...
				if (res.success)
				{
					Userform.editMode(res, true, ['control_helper', 'control_fileupload']);
				}
			}
		});
	},
	...
	setCustomHint: function (elem, value)
	{
		...
		{ //CCOEVENTS-179. We climb up to the nearest containing form.
	...
	fieldHasContent: function (id)
	{
		...
		var type = Userform.getInputType(id);
		switch (type)
		{
		case "address":
		case "combined":
		...
		case "birthdate":
			return Userform.getBirthDate(id);
		case "datetime":
			var date = Userform.getDateValue(id);
			return !(date == "T00:00" || date == '');
		case "time":
			return Userform.get24HourTime(id);
		case "checkbox":
		case "radio":
			return $$('#id_' + id + ' input').collect(function (e)
			{
	...
	{
		Userform.progressBar = new ProgressBar("progressBar",
		{
			'height': '20px',
			'width': '95%'
		});
		...
		var updateProgress = function ()
		{
			...
			$$('.form-line').each(function (el)
			{
				...
				{
					if (Userform.fieldHasContent(id))
					{
						completedFields++;
					}
				}
			...
			Userform.progressBar.setPercent(percentage);
	...
	{
		/*
			Note: The following was not handled on CWNP-557 because it is not presently a required feature for an embedded form.
			Being that this operation (this function) is for surveying/grading, this feature may likely never need updated functionality.
			The issue may rise that the form id (if this form is embedded and requires this functionality) may cause an error here.
		*/
		var formID = $$('input[name="formID"]')[0].value;
	...
	setInputTextMasking: function (elem, maskValue, unmask)
	{
		setTimeout(function ()
		{
			Userform.setQuestionMasking("#" + elem, 'textMasking', maskValue, unmask);
		}, 10);
	},
	setPhoneMaskingValidator: function (elem, maskValue, unmask)
	{
		setTimeout(function ()
		{
			Userform.setQuestionMasking("#" + elem, 'phoneMasking', maskValue, unmask);
		}, 10);
	}
};
function formIsQuickPost( form )
{
	/* This tests whether or not to bypass form validation on submit. */
	if( form && (form.qp || form.button_clicked == -1) )
		return true;
	else if( getQuerystring( 'qp' ) !== "" )
		return true;
	else
		return false;
}
...
window.fbAsyncInit = Userform.FBInit.bind(Userform);;
...
var BuildSource = {
	...
	{
		/*
			The code below properly displays and hides the various elements of the payment control based on the shliach's settings
			It also adds validate[required] to the classes of the displayed elements. This requires that input be entered into the displayed fields.
			Displayedoptions is an array because it can hold multiple parent elements --mshanowitz
		*/
		var payformExists = payformSettings.exists();
		if (payformExists) {
			if (payformSettings.options() != null) {
				var options = payformSettings.options();
				var isPayMeSelected = options.creditCard.payMe === true;
				var atLeastOneOptionIsEnabled = options.creditCard.enabled || options.eCheck.enabled || options.other.enabled || options.paypal.enabled;
				if (isPayMeSelected && !atLeastOneOptionIsEnabled)
					$$('#id_' + payformSettings.id())[0].hide();
			}
		}
		else {
			// SITES-1560 - Hide and unset processing fee offset donation if there is no pay form
			totalAmountHelper.offsetGiftToggleVisibility(false);
			//---
		}
		
		var selectedOptionValue = '';
		for(var x = 0; x < $$('.paymentMethod').length; x++)
		{
			if($$('.paymentMethod')[x].checked)
				selectedOptionValue = $$('.paymentMethod')[x].value.toLowerCase();
		}
		switch(selectedOptionValue)
		{
			case "creditcard":						
				...
				{
					this.displayedOptions = this.displayedOptions.concat($$(".credit_card"));
					for (var l = 0; l < $$('.credit_card').length; l++) {
						$$('.credit_card')[l].removeClassName('hide');
						// SITES-1560
						totalAmountHelper.offsetGiftToggleVisibility(true);
						//---
					}
				}
				if ($$('.billing_address').length > 0)
				{
					this.displayedOptions = this.displayedOptions.concat($$(".billing_address"));
					for (var l = 0; l < $$('.billing_address').length; l++)
						$$('.billing_address')[l].removeClassName('hide');
				}
				break;
			/*case "paypal":
				if ($$('.paypal').length > 0)
					for (var l = 0; l < $$('.paypal').length; l++)
						$$('.paypal')[l].removeClassName('hide');
				break;*/
			case "echeck":
				if ($$('.e_check').length > 0)
				{
					this.displayedOptions = this.displayedOptions.concat($$(".e_check"));
					for (var l = 0; l < $$('.e_check').length; l++) {
						$$('.e_check')[l].removeClassName('hide');
						// SITES-1560
						totalAmountHelper.offsetGiftToggleVisibility(false);
						//---
					}
			...
			case "other":
				if ($$('.other').length > 0)
				{
					this.displayedOptions = this.displayedOptions.concat($$(".other"));
					for (var l = 0; l < $$('.other').length; l++) {
						$$('.other')[l].removeClassName('hide');
						// SITES-1560
						totalAmountHelper.offsetGiftToggleVisibility(false);
						//---
					}
		...
		/* The code below properly displays and hides the various elements of the payment control based on what the user clicks.
		--mshanowitz
		*/
		...
		{
			var node;
			while (node = (this.displayedOptions.pop()))
				node.addClassName("hide");
		};
		this.showActive = function()
		{
			var node, i = 0;
			while (node = this.displayedOptions[i++])
		...
		this.paypal = function () {
			...
			this.displayedOptions = this.displayedOptions.concat($$(".paypal"));
			this.showActive();
			// SITES-1560
			totalAmountHelper.offsetGiftToggleVisibility(true);
			//---
...
}//end BuildSource object

var currency = "";
var	grandTotal = 0;
var pricingElementCount=0;
var selectedElements = {}

function updateValue (controlId)
{
	//CWNP-557 - controlIndex is a modification from parseInt(controlId).
	var controlIndex = controlId;
	if( controlIndex.indexOf( "-" ) > -1 )
		controlIndex = controlId.split( "-" )[1];
	if (isNaN(parseInt(controlIndex)))
			return;
	var controlType = formJson[controlId + '_type'];
	var control = document.getElementById('cid_' + controlId);
	var selectedElement = 0;
	switch (controlType)
	{
		case 'control_dropdown':
			if (!formJson[controlId + '_pricing'])
				return;

			pricingElementCount++;
			control = document.getElementById('input_' + controlId);
			var selectedIndex = parseInt(control.selectedIndex) - 1;
			if (selectedIndex >= 0)
				selectedElement = parseFloat(fixAmount(formJson[controlId + '_pricing'].split('|')[selectedIndex]));
			if (isNaN(selectedElement))
				selectedElement = 0;
			selectedElements[control.id] = selectedElement;
			break;
		case 'control_checkbox':
			if (!formJson[controlId + '_pricing'] && formJson[controlId + '_calculateOther'] != "Yes")
				return;

			pricingElementCount++;
			var options = control.getElementsByTagName('input');
			selectedElements[control.id] = 0;
			for (var a = 0; a < options.length; a++)
			{
				selectedElement = 0;
				if (options[a].checked)
				{
					if (options[a].id != 'other_' + controlId) {
						selectedElement = parseFloat(fixAmount(formJson[controlId + '_pricing']?.split('|')[a]));
					}
					else if (formJson[controlId + '_calculateOther'] == "Yes") {
						selectedElement = parseFloat(fixAmount(document.getElementById('input_' + controlId).value));
					}						
		...
		case 'control_radio':
			...
			for (var a = 0; a < options.length; a++)
			{
				if (options[a].checked)
				{
					if (options[a].id.split('_')[0] != 'other')
						selectedElement = parseFloat(fixAmount(formJson[controlId + '_pricing']?.split('|')[a]));
					else if (formJson[controlId + '_calculateOther'] == "Yes") {
						selectedElement = parseFloat(fixAmount(document.getElementById('input_' + controlId).value));
					}
		...
		case 'control_amount':
			pricingElementCount++;
			var options = control.getElementsByTagName('input');
			var mode = formJson[controlId + '_mode'];
			for (var a = 0; a < options.length; a++)
			{
				if (mode == 'textbox')
				{
					var amount = document.getElementById('input_' + controlId).value;
					selectedElement = parseFloat(fixAmount(amount));										 
					if (isNaN(selectedElement))
							selectedElement = 0;
					selectedElements[control.id] = selectedElement;
					break;
				...
				else
				{
					if (options[a].checked)
					{
						if (options[a].id.split('_')[0] === 'other')
						{
							//selectedElement = parseFloat(document.getElementById('input_' + controlId).value);
							var amount = document.getElementById('input_' + controlId).value;
							/*amount = amount.replace( /[^0-9\.\-\,]/g , '');
							selectedElement = parseFloat(amount);*/
							selectedElement = parseFloat(fixAmount(amount));
						}
						else
						{
							var amount = formJson[controlId + '_options'].split('|')[a];
							selectedElement = parseFloat(fixAmount(amount));
						}
		...
		case 'control_quantity':
			var value = control.getElementsByTagName('input')[0].value;
			if(value == '')
				value = '0';
			var perItem = formJson[controlId + '_pricePerItem'];
			if(perItem == '' || (typeof perItem == 'undefined' || perItem == null))
				perItem = '0';
			else
				pricingElementCount++;
			selectedElements[control.id] = (parseFloat(perItem) * parseFloat(value));
			break;
	}
}
...
function fixAmount(amount)
{
	...
	amount = amount.replace( /[^0-9\.\-\,]/g , '');
	var decimalPoint = amount.substring(amount.length - 3);
	if(decimalPoint[0] == ',' || decimalPoint[0] == '.') 
		decimalPoint = 2;	
	else if(decimalPoint[1] == ',' || decimalPoint[1] == '.') 
		decimalPoint = 1;
	else
		decimalPoint = 0;
	...
	if(decimalPoint > 0)
	{
		amount = parseFloat(amount / Math.pow(10, decimalPoint)).toFixed(2);
	}
...
{
	/**
	 * Track if any of the controls have an initial selected value.
	 * So that the calculated totals do include their values
	 * @type {boolean}
	 */
	let hasAmountControlsWithInitialValue = false;
	if( typeof ( initializedFormOptions ) == 'undefined' )
		initializedFormOptions = [];
	for (var key in formJson) // get the value of all pricing controls
	{
		if (key.split('_')[1] === 'type')
		{
			var controlId = key.split('_')[0];
			var checkbox = false;
			var selectedElement = document.getElementById( 'cid_' + controlId );
			var options = [];
			var els = ( !!selectedElement ? selectedElement.getElementsByTagName( 'input' ) : [] );
			for( var e = 0, eMax = els.length; e < eMax; e++ )
				options.push( els[e] );
			switch (formJson[key])
			{
				case 'control_quantity':
				case 'control_number':
				case 'control_dropdown':
					options = [document.getElementById('input_' + controlId)];
					// falls through
				case 'control_checkbox':
				case 'control_radio':
					if (!formJson[controlId + '_pricing'] && formJson[key] != 'control_number' && formJson[key] != 'control_quantity'
						&& formJson[controlId + '_calculateOther'] != "Yes")
					{
						break;
					}
					// falls through if conditions not met
				case 'control_amount':
					updateValue( controlId );
					for( var i = 0, iMax = options.length; i < iMax; i++ )
					{
						var option = options[i];
						if( initializedFormOptions.indexOf( option.id ) != -1 )
							continue;
						initializedFormOptions.push( option.id );
						var updateFunction = function (e) {
							Userform.lastFocus = this;
							updateValue(this.id.split('_')[1]);
							updateTotals();
						};
						Co.DomEvents.AddListener(option, 'change', updateFunction);
					}

					// track fields that have an initial selected value
					// so that the total can be updated to reflect those initial values.
					const selectedKey = `${controlId}_selected`;
					const isPreselected = !!formJson[selectedKey];
					if (isPreselected)
					{
						hasAmountControlsWithInitialValue = true;
					}
					break;					
			}
		}
	...
	var formContainersHaveATotalControl = $$( "#total_amount" ).length > 0;
	var payformMin = document.getElementById( "payformMin" );
	if( payformMin )
	{
		for( var key in formJson )
		{
	...
	updateTotalSchedular = new TaskSchedular(updateTotals, 50);
	if (pricingElementCount === 0)
	{
		$$('input[id*="_paypal"]').each(function(item){item.parentNode.hide();});
	}
	else
	{
		const isExcluded = (selectedElementKey) => {
			const fieldId = selectedElementKey.split('_')?.[1] ?? '';
			const fieldValue = `id_${fieldId}`
			return $j(`#${fieldValue}`)[0]?.classList.contains('form-field-hidden') ?? false;
		}
		
		updateTotals = function () {
			if (cachedValues === selectedElements)
				return;
			grandTotal = 0;
			for (var i in selectedElements) {
				...
				grandTotal += selectedElements[i];
			}
			if (formContainersHaveATotalControl && formContainers.length) {
				for (var f = 0; f < formContainers.length; f++) {
					var formContainer = formContainers[f];
					var runningTotal = 0;
					for (var s in selectedElements) {
						if (isExcluded(s)) continue;
						var selectedElement = document.getElementById(s);
						if (formContainer.contains(selectedElement))
							runningTotal += selectedElements[s];
					}
					// SITES-1560 - Calculate processing fee offset gift and add it to the total
					if (totalAmountHelper.exists() &&
						totalAmountHelper.offsetGiftChecked() &&
						runningTotal > 0) {
						var original = runningTotal;
						runningTotal += runningTotal * totalAmountHelper.offsetGiftPercent();
						runningTotal = Math.round(runningTotal * 100) / 100;
						totalAmountHelper.offsetGiftSetValue(runningTotal - original);
					} else {
						totalAmountHelper.offsetGiftSetValue(0);
					}
					//---
					if (payformSettings.exists() && payformSettings.id() > 0) {
						if (runningTotal > 0)
							Userform.requireField(payformSettings.id(), true);
						else if (runningTotal === 0 && !payformSettings.isAlwaysRequired())
							Userform.unrequireField(payformSettings.id());
					}

					var subTotal = formContainer.select('#total_amount').shift();
					if (subTotal)
						subTotal.innerHTML = formatCurrency(runningTotal);
				}
				if (multiplier > 1 && grandTotal > 0) {
					var text = 'Your donation will be worth {amount}'.locale().replace(/\{amount\}/gi, formatCurrency(grandTotal * multiplier));
					if ($$('#multiplier_message').length == 0) {
						$$("#total_amount").last().insert(
							{
								after: new Element('div', { 'class': 'top_padding', 'id': 'multiplier_message' }).update(text)
							});
					}
					else
						$$('#multiplier_message').first().update(text);
				}
			}
			else {
				if (payformSettings.exists() && payformSettings.id() > 0) {
					if (grandTotal > 0)
						Userform.requireField(payformSettings.id(), true);
					else if (grandTotal === 0 && !payformSettings.isAlwaysRequired())
						Userform.unrequireField(payformSettings.id());
				}
			}
			if (payformMin) {
				var total = 0;
				if (partialType === 'percent')
					total = grandTotal * percent / 100;
				else
					total = percent;
				if (grandTotal < total) // hide options if minimum is more than total
				{
					document.getElementById('payformWrapper').style.display = 'none';
				}
				else {
					document.getElementById('payformWrapper').style.display = 'block';
				}
				payformMin.innerHTML = parseFloat(total).toFixed(2);
			}
			$$('input[id*="_paypal"]').each(function (item) { item.disabled = (grandTotal == 0); });
			if (typeof (modifySubtotal) != 'undefined') {
				//CCOEVENTS-179.
				for (var key in selectedElements) {
					...
					var el = $(key);
					if (el)
						modifySubtotal(el, selectedElements);
				}
			}
			if (Userform.lastFocus) {
				//CWNP-557
				if (typeof (updateAmount) != 'undefined')
					updateAmount(Userform.lastFocus, grandTotal);
			}
		};
	}
	
	// make sure initial total includes the amounts from controls which have initial values
	if (hasAmountControlsWithInitialValue)
	{
		updateTotals();
	}
	updateTotalSchedular.setAction(updateTotals);
	updateTotalSchedular.runTask();
	return updateTotals;
}
...
{
	return formCurrency.symbol() + parseFloat(n).toFixed(2) + (formCurrency.isDefault() ? '' : ' ' + formCurrency.value());
}
...
function formatSize(bytes, decimals = 2) {
	...
	const fileSizeUnitIndex = Math.floor(Math.log(bytes) / Math.log(kilobyte));

	return `${parseFloat((bytes / Math.pow(kilobyte, fileSizeUnitIndex)).toFixed(fractionDigits))} ${fileSizeUnits[fileSizeUnitIndex]}`;
}
...
function validateNumber(evt) {
	...
	var isRequired = evt.target.name.endsWith("[other]") && $j(evt.target).closest(".form-radio-item").find(".form-radio-other").get(0).classList.contains('validate[required]');
	var isValid = !isRequired || parseFloat(evt.target.value + keyString) > 0;

	if ((!regex.test(keyString) && (key != 13)) || !isValid) {
...
//Simulates keyup event in the event that validation fails and the user had entered data into the pricing "other" text fields
var fireKeyUpEventOnPricingTextElements = function () {
	if (typeof arrayOfTextElementsToFire != 'undefined' && arrayOfTextElementsToFire.length > 0)
		for (var i = 0; i < arrayOfTextElementsToFire.length; i++) {
			$(arrayOfTextElementsToFire[i]).run('keyup');
		}
};
...
class TaskSchedular
{
	...
	constructor(action, throttleTimeMS) 
	{
		this.action = action;
	...
	runTask() {
		return new Promise((resolve) => {
			...
			const elapsedTime = currentTimestamp - (this.timestampOfLastRun ?? (currentTimestamp - this.throttleTimeMs))
			const hasEnoughTimeElapsed = elapsedTime >= this.throttleTimeMs;
			
			// if enough time has elapsed then the action is ran immediately
			if (hasEnoughTimeElapsed && this.hasAction())
			{
				this.action();
				this.timestampOfLastRun = currentTimestamp;
				resolve(true);
			}
			else 
			{
				// run action when throttle time has elapsed without any other requests
...
function isPaymentMethodOption(inputElement)
{
	return inputElement.type === 'radio' && inputElement.classList.contains('paymentMethod')
}